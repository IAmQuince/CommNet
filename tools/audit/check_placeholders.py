...
def main() -> int:
    ...
    scan_roots = [ROOT / "docs", ROOT / "registries", ROOT / "README.md", ROOT / "RELEASE_MANIFEST.md", ROOT / "RELEASE_NOTES.md"]
    files = []
    for s in scan_roots:
        if s.is_dir():
            files.extend([p for p in s.rglob("*") if p.is_file() and p.suffix.lower() in {".md", ".json", ".txt"}])
        elif s.is_file():
            files.append(s)
    for p in files:
        ...
        for name, pat in patterns:
            for m in pat.finditer(text):
                line = text.count("\n", 0, m.start()) + 1
                issues.append(f"{name} in {rel(p)}:{line}")
    lines.append(f"Files scanned: `{len(files)}`")
    lines += ["", "## Issues"]
    lines += [f"- {i}" for i in issues] if issues else ["- None"]
    return write_report("placeholder_scan.md", "Placeholder and Overclaim Scan", lines, not issues)
