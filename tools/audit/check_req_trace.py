...
def main() -> int:
    ...
    required_fields = ["id", "statement", "rationale", "verification", "status", "source_document", "implementation_target", "test_target"]
    valid_status = {"DRAFT", "READY_FOR_REVIEW", "READY_FOR_IMPLEMENTATION", "BLOCKED", "DEFERRED", "IMPLEMENTED", "VERIFIED"}
    for r in reqs:
        for f in required_fields:
            if not r.get(f):
                issues.append(f"Requirement {r.get('id')} missing {f}")
        if r.get("status") not in valid_status:
            issues.append(f"Requirement {r.get('id')} has invalid status {r.get('status')}")
        if r.get("status") in {"IMPLEMENTED", "VERIFIED"}:
            issues.append(f"Requirement {r.get('id')} claims {r.get('status')} in pre-code baseline")
    lines.append(f"Requirements checked: `{len(reqs)}`")
    lines += ["", "## Issues"]
    lines += [f"- {i}" for i in issues] if issues else ["- None"]
    return write_report("traceability_report.md", "Traceability Report", lines, not issues)
